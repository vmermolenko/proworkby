<template>
  <div>
    <v-container>
      <v-btn  @click="get_item_all()">Все заявки</v-btn>
      <v-row>
        <v-col cols="12" sm="6" md="4" v-for="item in orders" :key="item.id">
          <v-card class="mx-auto" raised>
            ...
            <v-card-actions>
              <v-spacer />
              <v-btn color="primary" @click="editItem(item)" >Взять в работу</v-btn>
            </v-card-actions>
          </v-card>
        ...
        <v-dialog v-model="dialogorder" max-width="500px">
          <v-card>
            <v-card-title>
              <span class="headline">Заказ №{{ editedItem.id }}</span>
            </v-card-title>
              ...
              <v-container>
                <v-row>
                  <v-col cols="12" sm="12" md="12">
                    <p class="body-1 text--primary">
                      Выберите количество рабочих которые могут поехать на заказ                      
                    </p>
                  </v-col>
                  <v-col cols="12" sm="12" md="12">
                    <v-select
                      :items='optionsCountWorkers'
                      v-model='editedItem.count_number_worker_edit'
                      item-text='text'
                      item-value='value'
                    />
                  </v-col>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="blue darken-1" text @click="dialogorder = false">Отмена</v-btn>
              <v-btn color="blue darken-1" text @click="editOrder(editedItem)">Подтвердить</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
...
export default {
  ...
  created () {
    ...
    this.postData.email = localStorage.email
  },
  ...
  methods: {
    ...
    async get_item_all(){
      axios
           .post('https://prowork.by/api_client/get_all_workerorders.php', this.postData, this.config)
    ...
    editItem (item) {
      ...
      { text: '2 Рабочих', value: '2' },
      { text: '3 Рабочих', value: '3' },
      { text: '4 Рабочих', value: '4' },
      { text: '5 Рабочих', value: '5' },
